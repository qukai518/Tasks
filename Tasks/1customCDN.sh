...
# 获取有效 config.sh 链接
get_valid_config() {
    config_list=(https://ghproxy.com/https://github.com/buqian123/kill/blob/main/Task/config.sample.sh)
...
# 获取有效 extra.sh 链接
get_valid_extra() {
    extra_list=(https://ghproxy.com/https://github.com/buqian123/kill/blob/main/Task/extra.sh)
...
# 获取有效 code.sh 链接
get_valid_code() {
    code_list=(https://ghproxy.com/https://github.com/buqian123/kill/blob/main/Task/code.sh)
...
get_valid_task_before() {
    task_before_list=(https://ghproxy.com/https://github.com/buqian123/kill/blob/main/Task/task_before.sh)
...
add_curl_sample() {
    if [ "$(grep -c "config.sample.sh" /ql/config/crontab.list)" != 0 ]; then
        ...
        # 获取token
        token=$(cat /ql/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"自动更新模板","command":"curl -L https://ghproxy.com/https://github.com/buqian123/kill/blob/main/Task/config.sample.sh -o /ql/sample/config.sample.sh && cp -rf /ql/sample/config.sample.sh /ql/config","schedule":"45 6,18 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1627380635389'
    fi
}
run_curl_sample() {

...
DOCKER_IMG_NAME="buqian/qinglong"
JD_PATH=""
SHELL_FOLDER=$(pwd)
CONTAINER_NAME=""
TAG="latest"
...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $CONFIG_PATH/auth.json)" != 0 ]; then
        ...
        log "8.开始青龙内部配置"
        docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://ghproxy.com/https://github.com/buqian123/kill/blob/main/Task/1customCDN.sh)"
    else
